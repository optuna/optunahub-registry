...
def perform_crossover(
    ...
    crowding_distances: defaultdict[int, float] | None = None,
) -> dict[str, Any]:
    numerical_search_space: dict[str, BaseDistribution] = {}
    categorical_search_space: dict[str, BaseDistribution] = {}
    ...
    while True:  # Repeat while parameters lie outside search space boundaries.
        parents = _select_parents(
            crossover, study, parent_population, rng, dominates, crowding_distances
        )
...
def _select_parent(
    ...
    if dominates(candidate0, candidate1, study.directions):
        return candidate0
    elif dominates(candidate1, candidate0, study.directions):
        return candidate1
    elif crowding_distances is not None:
        if crowding_distances[candidate0.number] > crowding_distances[candidate1.number]:
            return candidate0
        else:
            return candidate1
    else:
