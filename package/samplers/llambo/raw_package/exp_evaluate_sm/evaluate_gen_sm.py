...
if __name__ == "__main__":
    ...
    logger.info("=" * 200)
    logger.info(
        f"Evaluating Generative SM performance on {dataset} with {model} and {num_observed} observed configurations... Running {num_seeds} runs."
    )
    logger.info("=" * 200)
    ...
    for seed in range(num_seeds):
        ...
        # evaluate TPE
        scores = fit_and_predict_with_TPE(
            hp_constraints,
            observed_configs,
            observed_fvals,
            candidate_configs,
        ...
        corr, regret = evaluate_score(
            scores, candidate_fvals, observed_fvals, lower_is_better, top_pct=0.5
        )
        ...
        _, pred_probs, cost, time_taken = LLM_SM.select_query_point(
            observed_configs, observed_fvals, candidate_configs, True
        )

        tot_llm_cost += cost

        corr, regret = evaluate_score(
            pred_probs, candidate_fvals, observed_fvals, lower_is_better, top_pct=0.2
        )
        print(
            f"[LLAMBO] Correlation: {corr:.4f}, Regret: {regret:.4f}, Cost: ${cost:.4f}, Time: {time_taken:.4f}s"
        )
        logger.info(
            f"[LLAMBO] Correlation: {corr:.4f}, Regret: {regret:.4f}, Cost: ${cost:.4f}, Time: {time_taken:.4f}s"
        )
    ...
    logger.info("=" * 200)
    logger.info(
        f"[Evaluate Generative SM] {seed+1} evaluation runs complete! Total cost: ${tot_llm_cost:.4f}"
    )
