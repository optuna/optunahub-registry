...
def fit_and_predict_with_GP(hp_constraints, X_train, y_train, X_test):
    ...
    from smac.model.gaussian_process.kernels import ConstantKernel
    from smac.model.gaussian_process.kernels import MaternKernel
    from smac.model.gaussian_process.kernels import ProductKernel
    ...
    if X_train.shape[0] > 10:
        gp = GaussianProcess(
            cs, kernel, normalize_y=False, n_restarts=20 * int(X_train.shape[0] / 10), seed=0
        )
    else:
...
def fit_and_predict_with_SMAC(hp_constraints, X_train, y_train, X_test):
    ...
    y_pred, y_var = rf.predict(
        X_test, covariance_type="diagonal"
    )  # RF only works with diagonal, need to calculate std manually
    y_std = np.sqrt(y_var)
...
def fit_and_predict_with_TPE(
    hp_constraints, X_train, y_train, X_test, top_pct, multivariate, lower_is_better
):
    ...
    sampler = TPESampler(
        multivariate=multivariate,  # doing independent sampling if False, else multivariate
        consider_prior=True,
    ...
    for hp_name, hp_info in hp_constraints.items():
        ...
        if type == "int":
            if transform == ["log"]:
                distribution[hp_name] = optuna.distributions.IntDistribution(
                    hp_min, hp_max, log=True
                )
            else:
                distribution[hp_name] = optuna.distributions.IntDistribution(
                    hp_min, hp_max, log=False
                )
        elif type == "float":
            if transform in ["log"]:
                distribution[hp_name] = optuna.distributions.FloatDistribution(
                    hp_min, hp_max, log=True
                )
            else:
                distribution[hp_name] = optuna.distributions.FloatDistribution(
