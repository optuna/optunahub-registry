...
from exp_evaluate_sm.evaluate_sm_utils import fit_and_predict_with_GP
from exp_evaluate_sm.evaluate_sm_utils import fit_and_predict_with_SMAC
...
if __name__ == "__main__":
    ...
    logger.info("=" * 200)
    logger.info(
        f"Evaluating Disriminative SM performance on {dataset} with {model} and {num_observed} observed configurations... Running {num_seeds} runs."
    )
    logger.info("=" * 200)
    ...
    for seed in range(num_seeds):
        ...
        # evaluate GP
        y_pred, y_std = fit_and_predict_with_GP(
            hp_constraints, observed_configs, observed_fvals, candidate_configs
        )
        scores = evaluate_posterior(
            y_pred, y_std, candidate_fvals.to_numpy(), f_best, lower_is_better
        )
        rmse, r2, nll, mace, sharpness, observed_coverage, regret = scores
        logger.info(
            f"[GP] RMSE: {rmse:.4f}, R2 score: {r2:.4f}, NLL: {nll:.4f}, "
        ...
        # evaluate SMAC
        y_pred, y_std = fit_and_predict_with_SMAC(
            hp_constraints, observed_configs, observed_fvals, candidate_configs
        )
        scores = evaluate_posterior(
            y_pred, y_std, candidate_fvals.to_numpy(), f_best, lower_is_better
        ...
        rmse, r2, nll, mace, sharpness, observed_coverage, regret = scores
        logger.info(
            f"[SMAC] RMSE: {rmse:.4f}, R2 score: {r2:.4f}, NLL: {nll:.4f}, "
            f"Coverage: {observed_coverage:.4f}, MACE: {mace:.4f}, Sharpness: {sharpness:.4f}, Regret: {regret:.4f}"
        )
        ...
        y_mean, y_std, cost, time_taken = asyncio.run(
            LLM_SM._evaluate_candidate_points(observed_configs, observed_fvals, candidate_configs)
        )
        scores = evaluate_posterior(
        ...
        rmse, r2, nll, mace, sharpness, observed_coverage, regret = scores
        logger.info(
            f"[LLAMBO] RMSE: {rmse:.4f}, R2 score: {r2:.4f}, NLL: {nll:.4f}, "
        ...
        rmse, r2, nll, mace, sharpness, observed_coverage, regret = scores
        logger.info(
            f"[LLAMBO_VANILLA] RMSE: {rmse:.4f}, R2 score: {r2:.4f}, NLL: {nll:.4f}, "
            f"Coverage: {observed_coverage:.4f}, MACE: {mace:.4f}, Sharpness: {sharpness:.4f}, Regret: {regret:.4f} | Cost: ${cost:.4f}, Time: {time_taken:.4f}s"
        )
