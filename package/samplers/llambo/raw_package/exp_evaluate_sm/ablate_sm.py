...
def obtain_n_configurations(
    hp_constraints, n, dataset, model, task_metric, task_type, lower_is_better
):
    ...
    for i in range(5):
        ...
        study = optuna.create_study(
            sampler=optuna.samplers.TPESampler(seed=42 + i), direction=direction
        )
...
def sample_n_configurations(configs, scores, n, seed, existing_config=None):
    ...
    while number_sampled < n:
        ...
        sample_index = (
            scores.groupby("score_rank")
            .apply(lambda x: x.sample(1, random_state=seed + iter_i))
...
def evaluate_posterior(fval_pred, fval_pred_std, fval_true, f_best, lower_is_better):
    ...
    assert (
        len(fval_pred.shape) == 1 and len(fval_pred_std.shape) == 1 and len(fval_true.shape) == 1
    )
    ...
    nll = (
        0.5 * np.log(2 * np.pi * fval_pred_std**2)
        + 0.5 * ((fval_true - fval_pred) / fval_pred_std) ** 2
    )
    ...
    regret /= np.abs(
        max(fval_true.max(), observed_fvals.max().item())
...
if __name__ == "__main__":
    ...
    save_res_fpath = f"{script_dir}/results/ablate_sm/{dataset}/{model}/{num_observed}.json"
    if not os.path.exists(os.path.dirname(save_res_fpath)):
        os.makedirs(os.path.dirname(save_res_fpath))
    # define logging directory
    logging_fpath = f"{script_dir}/logs/ablate_sm/{dataset}/{model}/{num_observed}.log"
    ...
    logger.info("=" * 200)
    logger.info(
        f"Ablating SM performance on {dataset} with {model} and {num_observed} observed configurations... Executing {num_seeds} runs."
    )
    logger.info("=" * 200)
    ...
    logger.info("Collecting configurations - this might take a while...")
    sampled_configs, sampled_scores = obtain_n_configurations(
        hp_constraints,
        100,
        dataset,
        model,
    ...
    for seed in range(num_seeds):
        logger.info("=" * 200)
        logger.info(f"[LLAMBO] Ablating with seed {seed}...")

        observed_configs, observed_fvals = sample_n_configurations(
            sampled_configs, sampled_scores, num_observed, seed=seed
        )

        candidate_configs, candidate_fvals = sample_n_configurations(
            sampled_configs, sampled_scores, 10, seed=42
        )
        ...
        task_context["metric"] = (
            "mean squared error" if task_metric == "neg_mean_squared_error" else task_metric
        )
        ...
        y_mean, y_std, cost, time_taken = asyncio.run(
            LLM_SM._evaluate_candidate_points(
                observed_configs,
                observed_fvals,
                candidate_configs,
                use_context=False,
        ...
        scores = evaluate_posterior(
            y_mean, y_std, candidate_fvals.to_numpy(), f_best, lower_is_better
        )
        rmse, r2, nll, mace, sharpness, observed_coverage, regret = scores
        logger.info(
            f"[LLAMBO_Uninf] RMSE: {rmse:.4f}, R2 score: {r2:.4f}, NLL: {nll:.4f}, "
            f"Coverage: {observed_coverage:.4f}, MACE: {mace:.4f}, Sharpness: {sharpness:.4f}, Regret: {regret:.4f} | Cost: ${cost:.4f}, Time: {time_taken:.4f}s"
        )
        results["LLAMBO_Uninf"]["rmse"].append(rmse)
        results["LLAMBO_Uninf"]["r2"].append(r2)
        results["LLAMBO_Uninf"]["nll"].append(nll)
        results["LLAMBO_Uninf"]["mace"].append(mace)
        results["LLAMBO_Uninf"]["sharpness"].append(sharpness)
        results["LLAMBO_Uninf"]["observed_coverage"].append(observed_coverage)
        results["LLAMBO_Uninf"]["regret"].append(regret)
        results["LLAMBO_Uninf"]["y_pred"].append(y_mean.squeeze().tolist())
        results["LLAMBO_Uninf"]["y_std"].append(y_std.squeeze().tolist())
        results["LLAMBO_Uninf"]["y_true"].append(candidate_fvals.to_numpy().squeeze().tolist())
        results["LLAMBO_Uninf"]["llm_query_cost"].append(cost)
        results["LLAMBO_Uninf"]["llm_query_time"].append(time_taken)
    ...
    logger.info("=" * 200)
    logger.info(f"[LLAMBO] {seed+1} ablation runs complete! Total cost: ${tot_llm_cost:.4f}")
