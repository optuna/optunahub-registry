...
if __name__ == "__main__":
    ...
    save_res_dir = (
        f"{script_dir}/results_{sm_mode}/{dataset}/{model}/{ablation_type}_{shuffle_type}/"
    )
    if not os.path.exists(save_res_dir):
        os.makedirs(save_res_dir)
    # define logging directory
    logging_fpath = (
        f"{script_dir}/logs_{sm_mode}/{dataset}/{model}/{ablation_type}_{shuffle_type}.log"
    )
    ...
    for seed in range(num_seeds):
        logger.info("=" * 200)
        logger.info(
            f"Ablating LLAMBO ({sm_mode} | {ablation_type} | Shuffle features: {shuffle_features}) to tune {model} on {dataset} with seed {seed+1} / {num_seeds}..."
        )
        ...
        logger.info(
            f"[LLAMBO | Ablation: {ablation_type} | Shuffle features: {shuffle_features}] RUN COMPLETE, saved results to {save_res_dir}..."
        )
    ...
    logger.info("=" * 200)
    logger.info(
        f"[LLAMBO | Ablation: {ablation_type} | Shuffle features: {shuffle_features}] {seed+1} evaluation runs complete! Total cost: ${tot_llm_cost:.4f}"
    )
