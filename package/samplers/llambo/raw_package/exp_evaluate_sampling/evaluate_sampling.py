...
from exp_evaluate_sampling.evaluate_sampling_utils import sample_from_RandomSampler
from exp_evaluate_sampling.evaluate_sampling_utils import sample_from_TPESampler
from exp_evaluate_sampling.metrics_utils import calculate_gen_var
from exp_evaluate_sampling.metrics_utils import calculate_loglikelihood
from exp_evaluate_sampling.metrics_utils import calculate_mahalanobis_dist
        ...
        for hp_name, hp_info in hp_constraints.items():
            ...
            if hp_info[0] == "int":
                config[hp_name] = trial.suggest_int(
                    hp_name, hp_info[2][0], hp_info[2][1], log=use_log
                )
...
if __name__ == "__main__":
    ...
    save_res_fpath = (
        f"{script_dir}/results/evaluate_sampling/{dataset_name}/{model}/{num_observed}.json"
    )
    if not os.path.exists(os.path.dirname(save_res_fpath)):
        os.makedirs(os.path.dirname(save_res_fpath))
    # define logging directory
    logging_fpath = (
        f"{script_dir}/logs/evaluate_sampling/{dataset_name}/{model}/{num_observed}.log"
    )
    ...
    logger.info("=" * 200)
    logger.info(
        f"Evaluating sampling performance on {dataset_name} with {model} and {num_observed} observed configurations... Executing {num_seeds} runs."
    )
    logger.info("=" * 200)
    ...
    global_best_score = (
        global_perf[dataset_name]["global_min"]
        if lower_is_better
        else global_perf[dataset_name]["global_max"]
    )
    global_worst_score = (
        global_perf[dataset_name]["global_max"]
        if lower_is_better
        else global_perf[dataset_name]["global_min"]
    )
    logger.info(
        f"Global best score: {global_best_score:.4f}, global worst score: {global_worst_score:.4f}"
    )
    ...
    for seed in range(num_seeds):
        ...
        candidates = sample_from_TPESampler(
            hp_constraints, lower_is_better, 20, model_covariance, observed_configs, observed_fvals
        )
        # evaluate candidates
        ...
        logger.info(
            f"[TPE_IN] Average regret: {av_regret:.4f}, Best regret: {best_regret:.4f}, "
        ...
        av_regret, best_regret, perf_spread, ml_dist, gen_var, ll, candidate_scores = evaluation
        logger.info(
            f"[TPE_MULTI] Average regret: {av_regret:.4f}, Best regret: {best_regret:.4f}, "
            f"Score spread: {perf_spread:.4f}, Mahalanobis distance: {ml_dist:.4f}, "
            f"Gen var: {gen_var:.4f}, Log likelihood: {ll:.4f}"
        )
        ...
        av_regret, best_regret, perf_spread, ml_dist, gen_var, ll, candidate_scores = evaluation
        logger.info(
            f"[RANDOM] Average regret: {av_regret:.4f}, Best regret: {best_regret:.4f}, "
            f"Score spread: {perf_spread:.4f}, Mahalanobis distance: {ml_dist:.4f}, "
            f"Gen var: {gen_var:.4f}, Log likelihood: {ll:.4f}"
        )
        ...
        candidates, tot_cost, time_taken = LLM_Sampler.get_candidate_points(
            observed_configs, observed_fvals, alpha=-0.2
        )
        # evaluate candidates
        ...
        av_regret, best_regret, perf_spread, ml_dist, gen_var, ll, candidate_scores = evaluation
        logger.info(
            f"[LLAMBO] Average regret: {av_regret:.4f}, Best regret: {best_regret:.4f}, "
            f"Score spread: {perf_spread:.4f}, Mahalanobis distance: {ml_dist:.4f}, "
            f"Gen var: {gen_var:.4f}, Log likelihood: {ll:.4f}"
        )
