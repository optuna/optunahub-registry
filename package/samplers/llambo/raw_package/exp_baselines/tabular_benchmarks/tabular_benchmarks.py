...
DATA_DIR_NAME = os.path.join(os.getcwd(), "hpo_benchmarks")
SEEDS: Final = [665, 1319, 7222, 7541, 8916]
VALUE_RANGES = json.load(open(f"{CURRENT_DIR}/tabular_benchmarks.json"))
    ...
    def _fetch_discrete_config_space(self) -> CS.ConfigurationSpace:
        ...
        config_space.add_hyperparameters(
            [
                ...
                else CS.CategoricalHyperparameter(
                    name=name, choices=[str(i) for i in range(len(choices))]
                )
...
class HPOBench(AbstractBench):
    def __init__(self, model_name: str, dataset_id: int):
        super().__init__()
        ...
        data_path = os.path.join(
            DATA_DIR_NAME,
            "hpo-bench-%s" % model_name,
            str(dataset_id),
            f"{model_name}_{dataset_id}_data.parquet.gzip",
        )
        db = pd.read_parquet(
            data_path, filters=[(budget_name, "==", budget_value), ("subsample", "==", 1.0)]
        )
    ...
    def _search_dataframe(self, row_dict, df):
        ...
        idx = np.where(mask)
        assert len(idx) == 1, (
            "The query has resulted into mulitple matches. This should not happen. "
