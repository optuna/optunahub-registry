...
from bayesmark.data import load_data
from bayesmark.data import METRICS_LOOKUP
from bayesmark.data import ProblemType
import numpy as np
import pandas as pd
...
class SklearnModelCustom(TestFunction):
    ...
    _LIST_CLF_MAP = ["neg_log_loss", "accuracy", "balanced_accuracy", "f1_micro", "roc_auc"]

    _LIST_REG_MAP = ["neg_mean_absolute_error", "neg_mean_squared_error"]

    _CUSTOM_DATASET = ["maggic", "seer", "cutract", "Rosenbrock", "Griewank", "KTablet"]

    _LOG_DATASET = ["Rosenbrock", "Griewank", "KTablet"]
    ...
    def __init__(self, model, dataset, metric, shuffle_seed=0, data_root=None):
        ...
        assert metric in METRICS_LOOKUP[problem_type], (
            "Incompatible metric %s with problem type %s"
            % (
                metric,
                problem_type,
            )
        ...
        self.model_name = (
            SklearnModelCustom._LIST_MODELS_CLF[model]
            if self.is_classifier
            else SklearnModelCustom._LIST_MODELS_REG[model]
        )
        self.model_name2 = (
            SklearnModelCustom._LIST_MODELS_CLF2[model]
            if self.is_classifier
            else SklearnModelCustom._LIST_MODELS_REG2[model]
        )
    ...
    def get_config_space(self):
        ...
        for key in config.keys():
            is_log = (
                True
                if ("space" in config[key].keys() and config[key]["space"] in ["log", "logit"])
                else False
            )
            if config[key]["type"] == "real":
                CONFIG_SPACE.add_hyperparameters(
                    [
                        CS.UniformFloatHyperparameter(
                            name=key,
                            lower=config[key]["range"][0],
                            upper=config[key]["range"][1],
                            log=is_log,
                        )
                    ]
                )
                ...
                CONFIG_SPACE.add_hyperparameters(
                    [CS.CategoricalHyperparameter(key, [False, True])]
                )
            order_list += [key]
    ...
    def get_task_context(self):
        ...
        task_context["metric"] = (
            "accuracy" if task_dict["task"] == "classification" else "neg_mean_squared_error"
        )
    ...
    def evaluate(self, params):
        ...
        params = {
            key: params[key]
            if params[key] != "True" and params[key] != "False"
        ...
        if self.is_classifier:
            ...
            additional_metrics = self.compute_clf_metrics(
                y_pred, self.data_yt, y_pred_proba=y_pred_proba
            )
        else:
            # y_pred, y_pred_proba = self.obtain_pred(clf, self.data_Xt)
