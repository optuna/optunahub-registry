...
def bo_loop(
    ...
    for idx in range(n_repetitions):
        ...
        if bo_type in ["bo_hebo", "bo_turbo", "bo_tpe", "bo_smac", "bo_skopt", "bo_optuna"]:
            ...
            final_y, all_metrics = bo_used(
                fun_to_evaluate,
                config_space,
                n_runs=n_runs,
                n_init=n_init,
                seed=idx,
                config_init=list_init_config[idx],
                order_list=order_list,
            )
        else:
            BO_object = obtain_bo_object(bo_type, fun_to_evaluate, config_space, order_list, idx)
            final_y, all_metrics = BO_object.optimize(
                n_runs, n_init, config_init=list_init_config[idx]
            )
...
def run_bayesmark(
    list_model, list_data, list_metric, all_dict_templates, n_repetitions, n_runs, n_init
):
