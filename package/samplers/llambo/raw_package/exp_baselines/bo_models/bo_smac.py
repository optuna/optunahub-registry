...
def update_config_space_default_and_get_init_config(
    fun_to_evaluate, config_space, n_init, seed=0, config_init=None
):
    init_data = bo_tpe(
        fun_to_evaluate,
        config_space,
    ...
    init_configs = [
        CS.Configuration(
            config_space, values={hp_name: data[hp_name][i] for hp_name in config_space}
        )
...
def bo_smac(
    ...
    config_space, init_configs = update_config_space_default_and_get_init_config(
        fun_to_evaluate, config_space, n_init, seed, config_init
    )
    scenario = Scenario(
        {
            "run_obj": "quality",
