...
class BO_utils:
    def __init__(self, config_space, order_list):
        self.hyp_opt, self.hyp_type, self.hyp_log = self.extract_space_hyp(
            config_space, order_list
        )
...
class BO_optimization(nn.Module):
    ...
    def prepare_data_output(self, candidates, x_max, x_min):
        ...
        if self.bo_utils.hyp_log is not None:
            final_result = torch.where(
                self.bool_tensor.unsqueeze(0), final_result.exp(), final_result
            )
        return final_result
    ...
    def generate_initial_data(self, n_init, train_x):
        ...
        return (
            ...
            torch.tensor(exact_obj).reshape(-1, 1).float(),
            best_observed_value,
        )

    def optimize(self, n_runs, n_init, config_init):
        self.fun_to_evaluate.reset_results()
        init_x, init_y, best_init_y = self.generate_initial_data(
            n_init=n_init,
        ...
        for i in range(n_runs):
            ...
            # print(f"Before New candidates are: {new_candidates}")
...
from helper_funcs_sto_bnts import acq_max
from helper_funcs_sto_bnts import UtilityFunction
from jax import random as random_jax
import jax.numpy as np_jax


class BO_optimization_sto(BO_optimization):
    def __init__(self, fun_to_evaluate, config_space, order_list, seed=0, n_candidates=None):
        super().__init__(fun_to_evaluate, config_space, order_list, seed)
        self.util_ts = UtilityFunction()

    def get_next_points(self, init_x, init_y, best_init_y, n_points=1):
        init_x, init_y, bounds_torch, x_max, x_min = self.prepare_data_input(
            init_x, init_y, best_init_y
        )

        arr_transposed = np.transpose(bounds_torch.numpy())
        # Convert the transposed array to a list of tuples
        # bounds = [tuple(row) for row in arr_transposed]
        bounds = arr_transposed

        key = random_jax.PRNGKey(10)
        ensemble_key = random_jax.split(key, 1)
        Data = namedtuple("Data", ["inputs", "targets"])
        train_xs = np_jax.array(init_x.numpy())
        train_ys = np_jax.array(init_y.numpy().reshape(-1, 1))
        train_data = Data(inputs=train_xs, targets=train_ys)

        pred_model, pred_param = train_model(
            key=ensemble_key[0],
            train_method="ntkgp_param",
        ...
        x_obtained = acq_max(
            ac=self.util_ts.utility, pred_model=pred_model, pred_param=pred_param, bounds=bounds
        )
        candidates = torch.tensor(x_obtained)
        candidates = self.prepare_data_output(candidates, x_max, x_min)
        return candidates
