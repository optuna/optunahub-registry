...
from pybnn.bayesian_linear_regression import Prior
from pybnn.normalization import zero_mean_unit_var_denormalization
from pybnn.normalization import zero_mean_unit_var_normalization
...
class DNGO(BaseModel):
    ...
    def train(self, do_optimize=True):
        """
        ...
        self.network = Net(
            n_inputs=features, n_units=[self.n_units_1, self.n_units_2, self.n_units_3]
        )
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch in self.iterate_minibatches(self.X, self.y, batch_size, shuffle=True):
                inputs = torch.Tensor(batch[0]).float()
                targets = torch.Tensor(batch[1]).float()
        ...
        # Design matrix
        self.Theta = self.network.basis_funcs(torch.Tensor(self.X).float()).data.numpy()
        ...
        if do_optimize:
            if self.do_mcmc:
                ...
                if not self.burned:
                    ...
                    # Run MCMC sampling
                    result = self.sampler.run_mcmc(self.p0, self.burnin_steps, rstate0=self.rng)
                    self.p0 = result.coords

                    self.burned = True

                # Start sampling
                pos = self.sampler.run_mcmc(self.p0, self.chain_length, rstate0=self.rng)
        ...
        for sample in self.hypers:
            ...
            model = BayesianLinearRegression(alpha=sample[0], beta=sample[1], basis_func=None)
    ...
    def marginal_log_likelihood(self, theta):
        ...
        mll -= beta / 2.0 * np.linalg.norm(self.y - np.dot(self.Theta, m), 2)
    ...
    def iterate_minibatches(self, inputs, targets, batchsize, shuffle=False):
        assert inputs.shape[0] == targets.shape[0], "The number of training points is not the same"
        if shuffle:
    ...
    def posterior(self, X, **kwargs):
        ...
        # if len(X.shape) == 3:
        X = X.reshape(bs, -1)
        m, v = self.predict(X.float())
        m = torch.tensor(m).reshape(bs, 1, -1)
        v = torch.tensor(v).reshape(bs, 1, -1)
        return Posterior(m, v)
