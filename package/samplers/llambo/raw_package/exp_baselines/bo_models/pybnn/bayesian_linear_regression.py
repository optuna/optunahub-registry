...
class Prior(object):
    ...
    def sample_from_prior(self, n_samples):
        ...
        # Log alpha
        p0[:, 0] = self.rng.normal(loc=0, scale=1, size=n_samples)

        # Log sigma^2
        p0[:, 1] = self.rng.normal(loc=-3, scale=1, size=n_samples)
        return p0
...
class BayesianLinearRegression(BaseModel):
    ...
    def marginal_log_likelihood(self, theta):
        ...
        mll -= beta / 2.0 * np.linalg.norm(self.y - np.dot(self.X_transformed, m), 2)
    ...
    def train(self, X, y, do_optimize=True):
        ...
        if do_optimize:
            if self.do_mcmc:
                sampler = emcee.EnsembleSampler(self.n_hypers, 2, self.marginal_log_likelihood)
                ...
                if not self.burned:
                    ...
                    # Run MCMC sampling
                    result = sampler.run_mcmc(self.p0, self.burnin_steps, rstate0=self.rng)
                    self.p0 = result.coords

                    self.burned = True

                # Start sampling
                pos = sampler.run_mcmc(self.p0, self.chain_length, rstate0=self.rng)
    ...
    def predict(self, X_test):
        ...
        for i, h in enumerate(self.hypers):
            ...
            var[i] = 1.0 / h[1] + np.diag(
                np.dot(np.dot(X_transformed, self.models[i][1]), X_transformed.T)
            )
