        ...
        for group in self.param_groups:
            for parameter in group["params"]:
                ...
                if state["iteration"] <= group["num_burn_in_steps"]:
                    tau.add_(
                        -tau * (g * g / (v_hat + epsilon)) + 1
                    )  # specifies the moving average window, see Eq 9 in [1] left
                    g.add_(
                        -g * tau_inv + tau_inv * gradient
                    )  # average gradient see Eq 9 in [1] right
                    v_hat.add_(
                        -v_hat * tau_inv + tau_inv * (gradient**2)
                    )  # gradient variance see Eq 8 in [1]

                minv_t = 1.0 / (torch.sqrt(v_hat) + epsilon)  # preconditioner

                epsilon_var = 2.0 * (lr**2) * mdecay * minv_t - (lr**4)

                # sample random epsilon
                sigma = torch.sqrt(torch.clamp(epsilon_var, min=1e-16))
                sample_t = torch.normal(
                    mean=torch.zeros_like(gradient), std=torch.ones_like(gradient) * sigma
                )

                # update momentum (Eq 10 right in [1])
                momentum.add_(-(lr**2) * minv_t * gradient - mdecay * momentum + sample_t)
