    ...
    def func(trial: optuna.Trial) -> float:
        ...
        for hp in config_space.get_hyperparameters():
            ...
            if isinstance(hp, CS.CategoricalHyperparameter):
                ...
                eval_config[name] = trial.suggest_float(
                    name=name, low=hp.lower, high=hp.upper, log=hp.log
                )
...
def add_init_configs_to_study(study, bench, config_space, seed, n_init, config_init):
    ...
    for hp in config_space.get_hyperparameters():
        ...
        if isinstance(hp, CS.CategoricalHyperparameter):
            ...
            distributions[name] = optuna.distributions.FloatDistribution(
                low=hp.lower, high=hp.upper, log=hp.log
            )
        elif isinstance(hp, CS.UniformIntegerHyperparameter):
...
def bo_optuna(
    fun_to_evaluate, config_space, n_runs, n_init, seed=0, config_init=None, order_list=None
):
    if hasattr(fun_to_evaluate, "reseed"):
        fun_to_evaluate.reseed(seed)
    fun_to_evaluate.reset_results()
    sampler = optuna.samplers.TPESampler(
        multivariate=True, seed=seed, n_startup_trials=n_init, n_ei_candidates=20
    )
    study = optuna.create_study(sampler=sampler)
    add_init_configs_to_study(
        study=study,
        bench=fun_to_evaluate,
