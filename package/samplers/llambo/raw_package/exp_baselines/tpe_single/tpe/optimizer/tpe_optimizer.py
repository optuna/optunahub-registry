...
from tpe.optimizer.base_optimizer import ObjectiveFunc
...
class TPEOptimizer(BaseOptimizer):
    ...
    def sample(self) -> Dict[str, Any]:
        ...
        if self._tpe_sampler._multivariate:
            ...
            eval_config = {
                hp_name: config_cands[hp_name][best_idx]
                for dim, hp_name in enumerate(self._hp_names)
            }
        else:
            best_indices = np.argmax(pi_config, axis=-1)
            eval_config = {
                hp_name: config_cands[hp_name][best_indices[dim]]
                for dim, hp_name in enumerate(self._hp_names)
            }
