...
import numpy as np
from parzen_estimator import build_categorical_parzen_estimator
from parzen_estimator import build_numerical_parzen_estimator
from parzen_estimator import MultiVariateParzenEstimator
from parzen_estimator import ParzenEstimatorType
from tpe.utils.constants import CategoricalHPType
from tpe.utils.constants import config2type
from tpe.utils.constants import NumericalHPType
from tpe.utils.constants import NumericType
from tpe.utils.constants import WeightFuncType
...
class TreeStructuredParzenEstimator:
    def __init__(
        ...
        self._is_categoricals = {
            hp_name: self._config_space.get_hyperparameter(hp_name).__class__.__name__
    ...
    def _get_parzen_estimator(
        ...
        if is_categorical:
            top_lower = (
                ...
                else self._calculate_adapted_top(config, lower_vals.size)
            )
            top_upper = (
                self._top
                if self._top is not None
                else self._calculate_adapted_top(config, upper_vals.size)
            )
            kwargs.update(top=top_lower)
            pe_lower = build_categorical_parzen_estimator(
                vals=lower_vals, weights=weights_lower, **kwargs
            )
            kwargs.update(top=top_upper)
            pe_upper = build_categorical_parzen_estimator(
                vals=upper_vals, weights=weights_upper, **kwargs
            )
        else:
            kwargs.update(
            ...
            pe_lower = build_numerical_parzen_estimator(
                vals=lower_vals, weights=weights_lower, **kwargs
            )
            pe_upper = build_numerical_parzen_estimator(
                vals=upper_vals, weights=weights_upper, **kwargs
            )
