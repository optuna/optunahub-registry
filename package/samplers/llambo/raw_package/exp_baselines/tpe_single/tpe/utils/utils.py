...
from tpe.utils.constants import HPType
from tpe.utils.constants import NumericType
from tpe.utils.constants import type2config
...
class ParameterSettings(TypedDict):
    ...
    choices: Optional[
        List[str]
    ]  # The choices for categorical parameters. Must be str for config space
...
def get_args_from_parser(Choices: Enum, opts: Dict) -> Namespace:
    ...
    parser.add_argument(
        "--dataset", choices=list([c.name for c in Choices]), default=default_choice
    )  # type: ignore
...
def get_config_space(
    searching_space: Dict[str, ParameterSettings], hp_module_path: str
    ...
    for param_name, settings in searching_space.items():
        ...
        if settings["param_type"] not in type_choices:
            raise ValueError(
                f"`param_type` in params.json must have one of {type_choices}, "
                f"but got {settings['param_type']}."
            )

        if len(set(settings.keys()) - settings_keys) >= 1:
            raise ValueError(
                f"params.json must not include the keys not specified in {settings_keys}."
            )

        if "log" in settings.keys():
            if settings["log"] not in bool_json.__args__:  # type: ignore
                raise ValueError(
                    "`log` in params.json must have either True or False, "
                    f"but got {settings['log']}."
                )
