    ...
    for hp in config_space.get_hyperparameters():
        ...
        if isinstance(hp, CS.CategoricalHyperparameter):
            ...
            if choices[0] == False and choices == "True":
                str_result += (
                    " (boolean), " if not add_ranges else " (boolean between {false, true}), "
                )
            else:
                str_result += (
                    " (string), "
                    if not add_ranges
                    else " (string %s), " % str(choices).replace("[", "{").replace("]", "}")
                )
        else:
            ...
            if isinstance(hp, CS.UniformIntegerHyperparameter):
                str_result += (
                    " (integer), "
                    if not add_ranges
                    else f" (integer between [{lower}, {upper}]), "
                )
            elif isinstance(hp, CS.UniformFloatHyperparameter):
                str_result += (
                    " (float), " if not add_ranges else f" (float between [{lower}, {upper}]), "
                )
...
def add_context_information(
    text, config_space, num_recommendation=10, text_dict=None, provide_ranges=False
):
...
class TemplateReader(ABC):
    ...
    def add_context(self, config_space, num_recommendation=10, task_dict=None):
        ...
        text_with_context = add_context_information(
            text,
            config_space,
            num_recommendation=num_recommendation,
            text_dict=text_dict,
            provide_ranges=self.provide_ranges,
        )
...
class FullTemplate(TemplateReader):
    ...
    def obtain_dict_text(self, task_dict):
        ...
        if self.context_template == "Extended_Context":
            new_dict["[NUM_STAT]"] = self.add_str_num(
                task_dict["num_feat_cont"],
                str(task_dict["skew"])[1:-1],
                str(task_dict["kurtosis"])[1:-1],
            )
    ...
    def add_str_num(self, num_feat, skew, kurtosis):
        if num_feat > 0:
            this_str = (
                "We are standarizing numerical values to have mean 0 and std 1. The Skewness of each feature is %s and the kurtosis is %s."
                % (skew, kurtosis)
            )
        else:
            this_str = ""
