...
def check_value_range(hp_name: str, config: CSH.Hyperparameter, val: NumericType) -> None:
    if val < config.lower or val > config.upper:
        raise ValueError(
            f"The sampled value for {hp_name} must be [{config.lower},{config.upper}], but got {val}."
        )
...
def sample_initial_configuration(config_space, hp_names, is_categoricals, is_ordinals, rng):
    ...
    for hp_name in hp_names:
        ...
        eval_config[hp_name] = get_random_sample(
            hp_name=hp_name,
            rng=rng,
            ...
            CONFIG_SPACE.add_hyperparameters(
                [
                    CS.UniformFloatHyperparameter(
                        name=key, lower=kw_opt[key][0], upper=kw_opt[key][1], log=is_log
                    )
                ]
            )
...
def sample_configurations(config_space, seed, num_samples):
    ...
    is_categoricals = {
        hp_name: config_space.get_hyperparameter(hp_name).__class__.__name__
    ...
    for i in range(num_samples):
        list_config += [
            sample_initial_configuration(config_space, hp_names, is_categoricals, is_ordinals, rng)
        ]
    return list_config
...
def run(
    minimizer, bounds, num_samples, random_state, initial_point_generator="random", n_repeats=1
):
...
def obtain_space(config_space, order_list):
    ...
    from skopt.space import Categorical  # Import space definitions from skopt
    from skopt.space import Integer  # Import space definitions from skopt
    from skopt.space import Real  # Import space definitions from skopt
    ...
    for key in order_list:
        ...
        if isinstance(hp, CS.UniformFloatHyperparameter):
            ...
            new_space += [
                Real(hp.lower, hp.upper, prior=prior, name=hp.name, transform="identity")
            ]
        elif isinstance(hp, CS.UniformIntegerHyperparameter):
            prior = "log-uniform" if hp.log else "uniform"
            new_space += [
                Integer(hp.lower, hp.upper, prior=prior, name=hp.name, transform="identity")
            ]
        else:
...
def chat_gpt(input_text):
    ...
    resp = openai.ChatCompletion.create(
        engine=ENGINE, messages=message, temperature=0.7, top_p=0.95, n=30, request_timeout=100
    )
    return resp
...
def obtain_gpt_init(template_object, config_space, n_init, task_dict):
    input_text = template_object.add_context(
        config_space, num_recommendation=n_init, task_dict=task_dict
    )
...
def write_random_bayesmark(
    list_model, list_data, list_metric, n_init, num_seeds, template_object=None
):
    from bayesmark.sklearn_funcs import SklearnModelCustom
    from utils_templates import FullTemplate
            ...
            for this_loader in list_data:
                ...
                if isinstance(template_object, FullTemplate):
                    ...
                    if template_object is None:
                        list_config = [
                            sample_configurations(config_space, i, n_init)
...
def write_random_tabular(list_model, list_data, n_init, num_seeds, random_init=None):
    ...
    template_object = (
        RandomTemplate() if random_init is None else RandomTemplate(add_path="-%s" % random_init)
    )
    for data_id in list_data:
        for this_model in list_model:
            ...
            if random_init is None:
                list_config = [
                    sample_configurations(config_space, i, n_init) for i in range(num_seeds)
                ]
            else:
                list_config = [
                    obtain_dict_init(
                        random_init, config_space, order_list, random_state=i, num_samples=n_init
                    )
                    for i in range(num_seeds)
                ]
