...
class LLM_GEN_SM:
    ...
    async def _evaluate_candidate_points(
        ...
        # Ensure hyperparameter_constraints are properly formatted
        hyperparameter_constraints = self.task_context["hyperparameter_constraints"]
        for key, value in hyperparameter_constraints.items():
            if isinstance(value[2], list):  # If lower_bound is a list, take the first element
                hyperparameter_constraints[key] = (value[0], value[1], value[2][0])
