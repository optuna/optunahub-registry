...
class LLM_DIS_SM:
    def __init__(
        ...
        ), "Cannot do recalibration and boostrapping at the same time"
        ...
        for i in range(0, len(query_examples), 5):
            ...
            for _, sample_response in enumerate(chunk_results):
                if not sample_response:  # if sample prediction is an empty list :(
                    ...
                    sample_preds = []
                    for item in sample_response:  # Iterate over each sublist in sample_response
                        for gen_text in item:  # Iterate over each element in the sublist
                            # Ensure gen_text is a string (in case there are non-string elements like numbers)
