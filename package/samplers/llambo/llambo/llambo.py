...
class LLAMBO:
    ...
    def _initialize(self):
        ...
        assert isinstance(
            init_configs, list
        ), "init_f() should return a list of configs (dictionaries)"
        ...
        assert (
            init_configs.shape[0] == self.n_initial_samples
        ), "init_f() should return n_initial_samples number of configs"
        ...
        for index, _ in init_configs.iterrows():
            ...
            if self.observed_fvals.empty:
                ...
                self.observed_fvals = pd.concat(
                    [self.observed_fvals, one_result], axis=0, ignore_index=True
                )
            ...
            if self.observed_configs.empty:
                ...
                self.observed_configs = pd.concat(
                    [self.observed_configs, one_config], axis=0, ignore_index=True
                )
    ...
    def _evaluate_config(self, config):
        ...
        assert isinstance(
            eval_config, dict
        ), "bbox_eval_f() should return the evaluated config as a dictinoary"
        assert isinstance(
            eval_results, dict
        ), "bbox_eval_f() should return bbox evaluation results as a dictionary"
