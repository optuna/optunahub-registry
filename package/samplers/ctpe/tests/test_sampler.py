...
cTPESampler = optunahub.load_local_module(
    package="samplers/ctpe", registry_root="package/"
).cTPESampler


def dummy_constraints(trial: FrozenTrial) -> tuple[float]:
    return (0,)
...
parametrize_sampler = pytest.mark.parametrize(
    "sampler_class",
    [
        lambda: cTPESampler(n_startup_trials=0, constraints_func=dummy_constraints),
        lambda: cTPESampler(
            n_startup_trials=0, multivariate=True, constraints_func=dummy_constraints
        ),
    ],
)
...
parametrize_multi_objective_sampler = pytest.mark.parametrize(
    ...
    [
        lambda: optuna.samplers.TPESampler(n_startup_trials=0, constraints_func=dummy_constraints),
    ],
)


sampler_class_with_seed: dict[str, Callable[[int], BaseSampler]] = {
    "TPESampler": lambda seed: optuna.samplers.TPESampler(
        seed=seed, constraints_func=dummy_constraints
    ),
    "multivariate TPESampler": lambda seed: optuna.samplers.TPESampler(
        multivariate=True, seed=seed, constraints_func=dummy_constraints
    ),
}
    ...
    [
        (
            lambda: optuna.samplers.TPESampler(
                n_startup_trials=0, constraints_func=dummy_constraints
            ),
            True,
            True,
        ),
...
def test_raise_error_for_samplers_during_multi_objectives(
    ...
    with pytest.raises(ValueError):
        study.sampler.sample_relative(
            study, _create_new_trial(study), {"x": FloatDistribution(-5, 5)}
        )
