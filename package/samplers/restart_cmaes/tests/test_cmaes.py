...
# RestartCmaEsSampler = optunahub.load_local_module(
#     package="samplers/restart_cmaes", registry_root="package/"
# ).RestartCmaEsSampler


def test_consider_pruned_trials_experimental_warning() -> None:
    with pytest.warns(optuna.exceptions.ExperimentalWarning):
        RestartCmaEsSampler(consider_pruned_trials=True)


def test_with_margin_experimental_warning() -> None:
    with pytest.warns(optuna.exceptions.ExperimentalWarning):
        RestartCmaEsSampler(with_margin=True)


def test_lr_adapt_experimental_warning() -> None:
    with pytest.warns(optuna.exceptions.ExperimentalWarning):
        RestartCmaEsSampler(lr_adapt=True)


@pytest.mark.filterwarnings("ignore::optuna.exceptions.ExperimentalWarning")
@pytest.mark.parametrize(
    "use_separable_cma, cma_class_str",
    [(False, "restart_cmaes.cmaes.cmaes.CMA"), (True, "restart_cmaes.cmaes.cmaes.SepCMA")],
)
...
def test_should_raise_exception() -> None:
    ...
    with pytest.raises(ValueError):
        RestartCmaEsSampler(use_separable_cma=True, with_margin=True)
...
def test_incompatible_search_space(with_margin: bool) -> None:
    ...
    sampler = RestartCmaEsSampler(with_margin=with_margin, source_trials=source_study.trials)
...
def test_get_trials() -> None:
    with patch(
        ...
        sampler = RestartCmaEsSampler(consider_pruned_trials=False)
        study = optuna.create_study(sampler=sampler)
        trials = sampler._get_trials(study)
        assert len(trials) == 1

        sampler = RestartCmaEsSampler(consider_pruned_trials=True)
        study = optuna.create_study(sampler=sampler)
        trials = sampler._get_trials(study)
        assert len(trials) == 2
...
def test_internal_optimizer_with_margin() -> None:
    ...
    for objective in objectives:
        with patch("optuna.samplers._cmaes.cmaes.CMAwM") as cmawm_class_mock:
            sampler = RestartCmaEsSampler(with_margin=True)
            study = optuna.create_study(sampler=sampler)
...
def test_rdb_storage(with_margin: bool, storage_name: str) -> None:
    ...
    with StorageSupplier(storage_name) as storage:
        study = optuna.create_study(
            sampler=RestartCmaEsSampler(with_margin=with_margin),
            storage=storage,
        )
