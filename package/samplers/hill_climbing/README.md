...
def knapsack_objective(trial):
    """Knapsack-like problem: select items to maximize value while staying under weight limit."""
    items = {}
    for i in range(8):
    ...
    item_data = {
        "item_0": (2, 3), "item_1": (3, 4), "item_2": (4, 5), "item_3": (5, 6),
        "item_4": (1, 1), "item_5": (6, 9), "item_6": (7, 10), "item_7": (3, 5)
    }
    
    total_weight = sum(item_data[item][0] * selected for item, selected in items.items())
    total_value = sum(item_data[item][1] * selected for item, selected in items.items())
    
    # Constraint: maximum weight of 15
    if total_weight > 15:
        return -(total_weight - 15) * 100  # Heavy penalty for constraint violation
    
    return total_value  # Maximize value

module = optunahub.load_module("samplers/hill_climbing")
sampler = module.HillClimbingSampler(neighbor_size=10, max_restarts=6, seed=456)

study = optuna.create_study(sampler=sampler, direction="maximize")
study.optimize(knapsack_objective, n_trials=100)

print(f"Maximum value: {study.best_value}")
print(f"Selected items: {[k for k, v in study.best_params.items() if v == 1]}")
##Testing
##Running Tests
To execute the tests for HillClimbingSampler, run the following commands. The test suite is comprehensive and covers all major functionality:
...
##Sample test output:
console$ pytest package/samplers/hill_climbing/tests/ -v
========================= test session starts =========================
platform darwin -- Python 3.13.3, pytest-8.4.1, pluggy-1.6.0
cachedir: .pytest_cache
rootdir: /Users/user/optunahub-registry
configfile: pyproject.toml
collected 5 items

package/samplers/hill_climbing/tests/test_hill_climbing.py::TestHillClimbingSampler::test_initialization PASSED \[20%\]
package/samplers/hill_climbing/tests/test_hill_climbing.py::TestHillClimbingSampler::test_supported_distributions PASSED \[40%\]
package/samplers/hill_climbing/tests/test_hill_climbing.py::TestHillClimbingSampler::test_basic_optimization_minimize PASSED \[60%\]
package/samplers/hill_climbing/tests/test_hill_climbing.py::TestHillClimbingSampler::test_basic_optimization_maximize PASSED \[80%\]
package/samplers/hill_climbing/tests/test_hill_climbing.py::TestHillClimbingSampler::test_study_direction_support PASSED \[100%\]
...
Current best position and value (respecting optimization direction)
List of neighbors to evaluate
Set of already evaluated neighbors (to avoid re-evaluation)
Restart counter and direction-aware improvement tracking
...
python# ❌ This will raise an error
