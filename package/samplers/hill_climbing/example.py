...
def discrete_optimization_objective(trial: optuna.Trial) -> float:
    ...
    objective_value = (
        x**2 + y**2 + 
        algorithm_penalty[algorithm] + 
        preprocessing_bonus[preprocessing]
    )
...
def knapsack_like_objective(trial: optuna.Trial) -> float:
    ...
    total_weight = sum(item_properties[item][0] * selected 
                      for item, selected in items.items())
    total_value = sum(item_properties[item][1] * selected 
                     for item, selected in items.items())
