...
class Problem(optunahub.benchmarks.BaseProblem):
    ...
    def evaluate_constraints(self, params: dict[str, float]) -> dict[str, float]:
        ...
        if self._dtlz_type == {"constraint_type": 1, "function_id": 1}:
            constraints = [objective_values[-1] / 0.6 + sum(objective_values[:-1]) / 0.5 - 1.0]
            ...
            constraints = [-(sum_squares - 16) * (sum_squares - r**2)]
            ...
            constraints = [-sum_values - v + 1.0 for v in objective_values]
        elif self._dtlz_type == {"constraint_type": 3, "function_id": 4}:
            squares = [x**2 for x in objective_values]
            constraints = [-sum(squares) + v * 0.75 + 1.0 for v in objective_values]
        else:
