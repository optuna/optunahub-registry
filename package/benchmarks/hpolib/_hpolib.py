...
try:
    ...
    raise ModuleNotFoundError("Please run `pip install simple-hpo-bench` to use `hpolib`.")
...
def _extract_search_space(bench: HPOLib) -> dict[str, optuna.distributions.BaseDistribution]:
...
class Problem(optunahub.benchmarks.BaseProblem):
    available_metric_names: list[str] = HPOLib.available_metric_names
    available_dataset_names: list[int] = HPOLib.available_dataset_names
